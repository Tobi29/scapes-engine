 ...
 */
interface Elements<T> : ElementsRO<T>, VarsModifiableIterable<T> {
    /**
    ...
    override fun iterator(): ModifiableIterator<T> =
        object : SliceModifiableIterator<T>(size) {
            override fun access(index: Int) =
                get(index)

            override fun accessSet(index: Int, value: T) =
                set(index, value)
        }
}
 ...
 * @return Return value of [block]
 */
inline fun <reified T, R> Elements<T>.mutateAsArray(block: (Array<T>, Int, Int, () -> Unit) -> R): R {
    val array: Array<T>
    val offset: Int
...
inline fun <reified T, R> Elements<T>.mutateAsArray(block: (Array<T>, Int, Int) -> R): R =
    mutateAsArray { array, offset, size, close ->
        try {
