...
class InvalidOptionArgumentException(
        ...
        message: String = "Invalid option for ${option.simpleName} ${value.joinToString(
                " ")}"
) : InvalidCommandLineException(tokens, commandLine, message)
...
fun <E : InvalidTokensException> E.attach(
    ...
    is UnknownCommandException ->
        UnknownCommandException(tokens,
                command, message!!) as E
    is UnknownOptionException ->
        UnknownOptionException(tokens,
                option, message!!) as E
    is StrayArgumentException ->
        StrayArgumentException(tokens,
                argument, message!!) as E
    is MissingOptionArgumentException ->
        MissingOptionArgumentException(tokens, commandLine,
                token, message!!) as E
    is ExtraOptionArgumentException ->
        ExtraOptionArgumentException(tokens, commandLine,
                token, message!!) as E
    is MissingArgumentException ->
        MissingArgumentException(tokens, commandLine,
                argument, amount, message!!) as E
    is ExtraArgumentException ->
        ExtraArgumentException(tokens, commandLine,
                token, message!!) as E
    is MissingOptionException ->
        MissingOptionException(tokens, commandLine,
                option, message!!) as E
    is InvalidOptionArgumentException ->
        InvalidOptionArgumentException(tokens, commandLine,
                option, value, message!!) as E
    else -> throw IllegalArgumentException("Invalid exception: $this")
}

inline fun <R> withArgs(tokens: Iterable<String>? = null,
                        block: () -> R): R =
        try {
            block()
