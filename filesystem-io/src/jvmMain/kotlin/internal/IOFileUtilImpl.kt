...
internal object IOFileUtilImpl : FileUtilImpl {
    ...
    private data class FilePathImpl(val file: File) : FilePath {
        override fun compareTo(other: FilePath) = file.compareTo(other.toFile())

        override fun toString() = file.toString()

        override fun toUri() = file.toURI().toUri()
        ...
        override fun startsWith(other: FilePath) =
            file.startsWith(other.toFile())

        override fun relativize(other: FilePath) =
            file.relativeToOrNull(other.toFile())?.let { path(it) }

        override val fileName get() = path(File(file.name))

        override val parent get() = file.parentFile?.let { path(it) }

        override fun toAbsolutePath() = path(file.absoluteFile)
    }
