...
): FunctionStatement = function(name, listOf(*arguments))

fun function(
    name: String,
    arguments: List<Expression>
): FunctionStatement = FunctionStatement(name, arguments)

fun ret(
    value: Expression
): ReturnStatement = ReturnStatement(value)

fun Scope.declareIdentifier(
    name: String,
    type: Type
): Identifier = add(name, type)
...
fun Scope.declaration(
    ...
    val identifier = declareIdentifier(name, type)
...
class ParameterScope(
    ...
    fun add(
        ...
        val identifier = scope.declareIdentifier(name, type)
...
class ShaderProgramScope(val scope: Scope = Scope()) {
    ...
    private val shaders =
        HashMap<String, Pair<Scope, (Scope) -> ShaderFunction>>()
    ...
    fun declaration(
        type: Type,
        name: String,
        initializer: Scope.() -> Expression? = { null }
    ): Identifier {
        val identifier = scope.declareIdentifier(name, type)
        declarations.add(
    ...
    fun finish() = ShaderProgram(
        declarations, functions, shaders, outputs,
        uniforms, properties
    ).finish(scope)
}
