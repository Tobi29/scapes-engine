...
public abstract class AbstractCoroutine<in T>(
                ...
                is Cancelled -> return // ignore resumes on cancelled continuation
                    ...
                    if (ignoreRepeatedResume) {
                        ...
                        throw IllegalStateException(
                                "Already resumed, but got value $value")
                }
            }
        }
    }

    final override fun resumeWithException(exception: Throwable) = resumeWithException(
            exception, defaultResumeMode)

    protected fun resumeWithException(exception: Throwable,
                                      mode: Int) {
        while (true) {
            // lock-free loop on state
            val state = this.state // atomic read
            when (state) {
                ...
                is Cancelled -> {
                    ...
                    // TODO throw IllegalStateException("Already resumed, but got exception $exception", exception)
                        throw IllegalStateException(
                                "Already resumed, but got exception $exception")
                }
            }
        }
    }
    ...
    /* TODO: // for nicer debugging
    override fun toString(): String {
        val state = this.state
        val result = if (state is Incomplete) "" else "[$state]"
        return "${this::class.java.simpleName}{${stateToString(
                state)}}$result@${Integer.toHexString(
