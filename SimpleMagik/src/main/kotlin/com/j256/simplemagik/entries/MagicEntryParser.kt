...
object MagicEntryParser {
    ...
    fun parseLine(
        ...
        val parts = splitLine(line, errorCallBack) ?: return null
        ...
        if (offsetString.length == 0) {
            errorCallBack?.invoke(
                line,
                "invalid offset number:" + offsetString,
                null
            )
            return null
        }
        ...
        if (sindex >= 0) {
            ...
            try {
                ...
                errorCallBack?.invoke(
                    line,
                    "invalid type AND-number: " + andStr,
                    e
                )
                return null
            }
        ...
        var matcher = MagicType.matcherfromString(typeStr)
        if (matcher == null) {
            if (typeStr[0] == 'u') {
                ...
                if (index > 0) {
                    matcher = MagicType.matcherfromString(
                        typeStr.substring(
                            0,
                            index
                        )
            ...
            if (matcher == null) {
                errorCallBack?.invoke(
                    line,
                    "unknown magic type string: " + typeStr,
                    null
                )
                matcher = UnknownType
            }
        ...
        if (testStr == "x") {
            ...
            try {
                ...
                errorCallBack?.invoke(
                    line,
                    "could not convert magic test string: " + testStr,
                    e
                )
                return null
            }
    ...
    private fun splitLine(
        ...
        return if (startPos < 0) {
            arrayOf(levelStr, typeStr, testStr)
        } else {
            // format is the rest of the line
            arrayOf(levelStr, typeStr, testStr, line.substring(startPos))
        }
    }
