...
public suspend fun <T> withTimeout(time: Long, unit: TimeUnit = TimeUnit.MILLISECONDS, block: suspend () -> T): T {
    ...
    if (time <= 0L) throw java.util.concurrent.CancellationException("Timed out immediately")
...
) : java.util.concurrent.CancellationException("Timed out waiting for $time $unit")