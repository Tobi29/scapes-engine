...
public abstract class AbstractSendChannel<E> : SendChannel<E> {
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    protected fun describeSendBuffered(element: E): AddLastDesc<*> = SendBufferedDesc(
            queue, element)

    private open class SendBufferedDesc<out E>(
        queue: LockFreeLinkedListHead,
        element: E
    ) : AddLastDesc<SendBuffered<E>>(queue,
            SendBuffered(
                    element)) {
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    protected fun describeSendConflated(element: E): AddLastDesc<*> = SendConflatedDesc(
            queue, element)
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    protected fun describeTryOffer(element: E): TryOfferDesc<E> = TryOfferDesc(
            element, queue)
    ...
    private inner class TryEnqueueSendDesc<E, R>(
        ...
        block: suspend () -> R
    ) : AddLastDesc<SendSelect<R>>(queue,
            SendSelect(
...
public abstract class AbstractChannel<E> : AbstractSendChannel<E>(), Channel<E> {
    ...
    private suspend fun receiveSuspend(): E = suspendAtomicCancellableCoroutine(holdCancellability = true) sc@ { cont ->
        val receive = ReceiveElement(
                cont as CancellableContinuation<E?>, nullOnClose = false)
        while (true) {
    ...
    public final override fun iterator(): ChannelIterator<E> = Itr(
            this)
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    protected fun describeTryPoll(): TryPollDesc<E> = TryPollDesc(
            queue)
    ...
    private class ReceiveHasNext<E>(
        ...
        override fun tryResumeReceive(value: E, idempotent: Any?): Any? {
            ...
            if (token != null) {
                 ...
                 */
                if (idempotent != null) return IdempotentTokenValue(
                        token, value)
...
public class Closed<in E>(
    ...
    val receiveException: Throwable get() = closeCause ?: ClosedReceiveChannelException(
            DEFAULT_CLOSE_MESSAGE)
