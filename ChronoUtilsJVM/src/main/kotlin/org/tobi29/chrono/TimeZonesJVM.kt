...
class ThreeTenTimeZone(val ttZone: TTZoneId) : TimeZone {
    override fun encodeWithOffset(instant: InstantNanos): Set<OffsetDateTime> =
        try {
            setOf(
                instant.ttInstant.atZone(
                    ttZone
                ).toOffsetDateTime().chOffsetDateTime
            )
        } catch (e: Exception) {
            emptySet()
        }
...
val TTLocalDate.chDate
    get() = Date(
        year = year, month = Month.ofValue(monthValue),
        day = dayOfMonth
    )
